...
const barberSchema = Joi.object({
  ...
  payment_method: Joi.string().valid("tf", "qris").optional(),
...
const addBarber = async (req, res) => {
  ...
  try {
    ...
    const result = await executeQuery(
      "INSERT INTO barbers (name, latitude, longitude, services, paket, paket_description, price, profile_image, bank_name, account_number, payment_method, no_telp) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
      [
        name,
        latitude,
        longitude,
...
const updateBarber = async (req, res) => {
  ...
  try {
    ...
    const result = await executeQuery(
      "UPDATE barbers SET name = ?, services = ?, paket = ?, paket_description = ?, price = ?, profile_image = ?, bank_name = ?, account_number = ?, payment_method = ?, no_telp = ? WHERE id = ?",
      [
        updatedData.name,
...
const deleteBarber = async (req, res) => {
  ...
  try {
    ...
    await executeQuery(
      "DELETE FROM payment_uploadproof WHERE payment_id IN (SELECT id FROM payments WHERE barber_id = ?)",
      [id]
    );
    await executeQuery("DELETE FROM payments WHERE barber_id = ?", [id]);
